...
static void mavlink_test_self_fax_formation(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_SELF_FAX_FORMATION >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_self_fax_formation_t packet_in = {
        123.0,179.0,235.0,291.0,347.0,403.0
    };
    mavlink_self_fax_formation_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.ned_vel_x = packet_in.ned_vel_x;
        packet1.ned_vel_y = packet_in.ned_vel_y;
        packet1.ned_vel_z = packet_in.ned_vel_z;
        packet1.latitude = packet_in.latitude;
        packet1.altitude = packet_in.altitude;
        packet1.longtitude = packet_in.longtitude;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_SELF_FAX_FORMATION_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_SELF_FAX_FORMATION_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_self_fax_formation_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_self_fax_formation_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_self_fax_formation_pack(system_id, component_id, &msg , packet1.ned_vel_x , packet1.ned_vel_y , packet1.ned_vel_z , packet1.latitude , packet1.altitude , packet1.longtitude );
    mavlink_msg_self_fax_formation_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_self_fax_formation_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.ned_vel_x , packet1.ned_vel_y , packet1.ned_vel_z , packet1.latitude , packet1.altitude , packet1.longtitude );
    mavlink_msg_self_fax_formation_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_self_fax_formation_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_self_fax_formation_send(MAVLINK_COMM_1 , packet1.ned_vel_x , packet1.ned_vel_y , packet1.ned_vel_z , packet1.latitude , packet1.altitude , packet1.longtitude );
    mavlink_msg_self_fax_formation_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_self_fax_formation(system_id, component_id, last_msg);
